...
import { Text, ScrollView , useWindowDimensions, View, TouchableOpacity, StyleSheet, Dimensions, Alert } from 'react-native';
...
import Animated, { Easing, SharedTransition, useAnimatedStyle, useSharedValue, withSpring, withTiming } from 'react-native-reanimated';
  ...
  return (
    <>
      ...
      <ScrollView style={{ flex: 1, paddingBottom: 20 }} showsVerticalScrollIndicator={false}>
          <Animated.Image sharedTransitionTag={'image' + item.id}  source={{ uri: item.thumbnail }} style={{ height: dimen.height * 0.5, width: dimen.width, resizeMode: 'stretch' }} />
      ...
      <View style={{ flexDirection: 'row', backgroundColor: 'transparent', marginHorizontal: 30, position: 'absolute', bottom: 10 }}>
        <Animated.View style={[{ flex: 0.3, marginBottom: 10, backgroundColor: 'white', elevation: 10, borderRadius: 10, alignContent: 'center', justifyContent: 'center' }, bagAnimatedStyle]}>
          <Image
            source={require('../assets/icons/bag.png')}
            style={[{ width: 27, height: 27, tintColor: 'black', alignSelf: 'center', justifyContent: 'center' }]}
          />
        </Animated.View>
        <TouchableOpacity activeOpacity={sizeSelected && colorSelected ? 0.3 : 1} onPress={() => {
          if (!addedToCart && colorSelected && sizeSelected) {
            ...
            setTimeout(() => {
              ...
              setTimeout(() => {
                ...
                setTimeout(() => {
                  dispatch(addCart({ id: item?.id, c: displaycolor, s: displaySize }));
                  updateAdd(true);
                }, 100);
              }, 100);
            }, 600);
          ...
          style={{ flex: 0.7, marginBottom: 10, backgroundColor: sizeSelected && colorSelected ? '#DB3022' : '#EEB2B2', elevation: 10, marginHorizontal: 10, borderRadius: 10, alignContent: 'center' }}>
          <Text style={{ justifyContent: 'center', alignSelf: 'center', fontSize: 20, color: 'white', fontFamily: 'Urbanist-Regular', paddingVertical: 10 }}>Add to cart</Text>
        </TouchableOpacity>
      </View>
      <Animated.View style={[{
        backgroundColor: '#DB3022', width: 25, height: 25, borderRadius: 15,
        alignItems: 'center', justifyContent: 'center', position: 'absolute', bottom: 30, alignSelf: 'center',
      }, animatedStyle]}>
        <Text style={{ color: 'white', fontFamily: 'Urbanist-Bold', textAlign: 'center', textAlignVertical: 'center' }}>+1</Text>
      </Animated.View>
    </>
  );
};
